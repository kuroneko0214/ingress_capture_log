...
def get_ts_duration():
    ...
    try:
        ...
        query = get_query(name="get_failure_comm")
        cnt = cursor.execute(query)
        if cnt == 1:
            r = cursor.fetchall()
            mints, maxts = int(r[0][0]), int(r[0][1])
    except Exception as e:
        print(query)
        raise e
    finally:
...
def log_sync_failure(mints, maxts):
    try:
        db, cursor = connect_to_db()
        query = get_query(name="log_comm_failure").format(mints, maxts)
        cursor.execute(query)
        db.commit()
    except Exception as e:
        print(query)
...
def update_sync_status(mints, maxts):
    try:
        db, cursor = connect_to_db()
        query = get_query(name="update_sync_status").format(mints, maxts)
        cursor.execute(query)
        db.commit()
    except Exception as e:
        print(query)
